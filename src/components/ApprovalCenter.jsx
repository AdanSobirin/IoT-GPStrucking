...
// URL server PHP tempat foto upload disimpan (beda dari VITE_API_BASE_URL yang menunjuk FastAPI).
// Diatur lewat .env supaya sama antara lokal (XAMPP) dan VPS tanpa mengubah kode.
const PHP_SERVER_URL = import.meta.env.VITE_PHP_SERVER_URL || "http://localhost/dashboard1";
...
export default function ApprovalCenter() {
  ...
  useEffect(() => {
    ...
    let interval;
    if (activeTab === 'queue') {
      interval = setInterval(loadData, POLLING_INTERVAL_MS);
    }
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 h-full overflow-y-auto bg-gray-50 text-slate-900 dark:bg-slate-950 dark:text-slate-200 custom-scrollbar">


      {/* ─── HEADER ─── */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6 sm:mb-8">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Verifikasi Muatan PKS</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Otorisasi bongkar muat dan riwayat Tandan Buah Segar (TBS).</p>
        </div>
      ...
      <div className="flex bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-1">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg text-sm font-bold transition-all ${
      ...
      {isLoading ? (
              ...
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">
                      {req.driver} <span className="text-slate-400 dark:text-slate-600 mx-2">•</span> <span className="font-mono text-emerald-600 dark:text-emerald-400">{req.plate}</span>
                    </h3>
                ...
                <div className="flex gap-6 text-center bg-slate-100/70 dark:bg-slate-950 px-6 py-3 rounded-xl border border-slate-200/70 dark:border-slate-800/50">
                  ...
                  <div>
                    ...
                    <p className="text-lg font-bold text-slate-800 dark:text-slate-200">{req.janjang}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-emerald-600 dark:text-emerald-400 mt-1">Review ➔</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-lg mt-4 overflow-hidden">
          <div className="p-5 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50">
            <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">Log Approval Muatan</h3>
          </div>
                ...
                {history.length === 0 ? (
                  ...
                  history.map((item) => (
                    <tr
                      ...
                      className="border-b border-slate-200/70 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800/80 cursor-pointer transition-colors"
                    >
                      <td className="px-5 py-4">
                        <div className="font-mono text-emerald-600 dark:text-emerald-400 font-bold mb-1">APP-{item.id}</div>
                        <div className="text-[10px] text-slate-500">{item.time}</div>
                      </td>
                      <td className="px-5 py-4">
                        <div className="font-bold text-slate-800 dark:text-slate-200 mb-1">{item.driver}</div>
      ...
      {selectedReq && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex justify-between items-center p-5 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50">
              ...
              <button onClick={() => setSelectedReq(null)} className="text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors">
                <XCircle size={24} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto flex-1 custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-base font-bold text-slate-900 dark:text-white">{selectedReq.driver}</p>
                  <p className="text-sm text-emerald-600 dark:text-emerald-400 font-mono">{selectedReq.plate}</p>
                </div>
                <div className="w-full h-px bg-slate-200 dark:bg-slate-800"></div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <p className="text-xs text-slate-500 mb-1 flex items-center gap-1"><MapPin size={12}/> TPH Asal</p>
                    <p className="font-bold text-slate-800 dark:text-slate-200">{selectedReq.tph}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 mb-1 flex items-center gap-1"><Hash size={12}/> Blok</p>
                    <p className="font-bold text-slate-800 dark:text-slate-200">{selectedReq.blok}</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 mb-1 flex items-center gap-1"><Package size={12}/> Jumlah Janjang</p>
                    <p className="text-xl font-bold text-amber-500 dark:text-amber-400">{selectedReq.janjang}</p>
                  </div>
                  <div>
                    ...
                    <p className="font-bold text-slate-800 dark:text-slate-200 text-xs">{selectedReq.time}</p>
                  </div>
                </div>
              </div>

              <div className="bg-slate-100 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center p-2 min-h-[250px] relative overflow-hidden">
                {selectedReq.photo ? (
                  <img
            ...
            <div className="p-5 border-t border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-950/50 flex justify-end gap-3 items-center">
              <button
                ...
                className="px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
              >
                Tutup
              </button>
