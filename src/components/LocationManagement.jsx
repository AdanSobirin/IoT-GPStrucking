  ...
  return (
    <div className="p-8 h-full overflow-y-auto bg-slate-950 text-slate-200 custom-scrollbar">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-2xl font-bold text-white">Manajemen Lokasi</h2>
          <p className="text-slate-400 text-sm mt-1">Kelola titik TPH dan Pabrik Kelapa Sawit (PKS).</p>
        </div>
        <button 
          onClick={openAddModal}
          className="px-5 py-2.5 bg-emerald-500 text-slate-950 rounded-lg text-sm font-bold shadow-lg shadow-emerald-500/20 hover:bg-emerald-400 transition-colors flex items-center gap-2"
        >
      ...
      {/* ─── KONTEN TABEL ─── */}
      {isLoading ? (
        <div className="flex items-center gap-2 text-slate-400"><Zap className="animate-pulse text-emerald-500" /> Memuat data lokasi...</div>
      ) : error ? (
        ...
        <div className="bg-[#0f172a] border border-slate-800 rounded-2xl shadow-lg overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="text-[11px] text-slate-500 uppercase tracking-wider font-bold bg-slate-900/50">
                <tr>
              ...
              <tbody className="text-slate-300 text-xs">
                {locations.length === 0 ? (
                  ...
                  locations.map((loc) => (
                    <tr key={loc.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                      <td className="px-6 py-4 font-mono text-slate-500">LOC-{loc.id}</td>
                      <td className="px-6 py-4 font-bold text-slate-200">{loc.name}</td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => openEditModal(loc)} className="p-2 text-blue-400 hover:bg-slate-800 rounded transition-colors mr-2">
                          <Edit size={16} />
                        </button>
                        <button onClick={() => handleDelete(loc.id)} className="p-2 text-red-400 hover:bg-slate-800 rounded transition-colors">
                          <Trash2 size={16} />
                        </button>
                      </td>
                    </tr>
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col">
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-300">Nama Lokasi</label>
                  <input
                    type="text"
                    value={formData.name}
                    ...
                    className="w-full px-3 py-2 border rounded-lg bg-slate-950 border-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-400 text-white"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-300">Tipe Lokasi</label>
                  <select
                    value={formData.type}
              ...
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800">
                ...
                {/* WADAH PETA */}
                <div className="h-[250px] w-full rounded-lg overflow-hidden border border-slate-700 mb-4 z-0">
                  <MapContainer 
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border rounded-lg bg-slate-900 border-slate-800 text-sm text-slate-400 cursor-not-allowed"
                    />
                  </div>
                  <div>
              ...
              <div className="flex justify-end gap-3 mt-2">
                <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-sm font-bold text-slate-400 bg-slate-800 rounded-lg hover:bg-slate-700 transition-colors">
                  Batal
                </button>
